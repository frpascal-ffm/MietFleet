...
export function FahrtNeu() {
  const navigate = useNavigate()
  const { addFahrt, fahrer, fahrzeuge } = useData()
  ...
  const handleSave = (action: 'save' | 'save-and-new' | 'save-and-duplicate') => {
    // Basic validation
    ...
    if (action === 'save') {
      ...
      setNotes("")
      window.scrollTo(0, 0)
    } else if (action === 'save-and-duplicate') {
      // Keep everything, maybe just increment date or keep as is
      window.scrollTo(0, 0)
      alert("Fahrt gespeichert. Sie können nun die duplizierten Daten anpassen.")
    }
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Tragen Sie hier alle Details zur neuen Fahrt ein.</p>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4">
          ...
          <Button variant="secondary" onClick={() => handleSave('save-and-duplicate')} className="w-full sm:w-auto">
            Speichern & Duplizieren
          </Button>
          <Button variant="secondary" onClick={() => handleSave('save-and-new')} className="w-full sm:w-auto">
            Speichern & Weitere
          </Button>
          <Button onClick={() => handleSave('save')} className="w-full sm:w-auto">
            Speichern
          </Button>
        </div>
      </div>
