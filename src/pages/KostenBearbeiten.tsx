...
export function KostenBearbeiten() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { getKosten, updateKosten, fahrzeuge } = useData()

  const kostenItem = getKosten(id || "")
  ...
  useEffect(() => {
    if (kostenItem) {
      setType(kostenItem.type)
      setCategory(kostenItem.category)
      setAmount(kostenItem.amount)
      setCarId(kostenItem.carId)
      setDate(kostenItem.date)
      setNotes(kostenItem.notes || "")
    }
  }, [kostenItem])
  ...
  if (!kostenItem) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4">
        <h2 className="text-xl font-semibold text-gray-700">Kosten nicht gefunden</h2>
        <Button onClick={() => navigate("/kosten")}>Zurück zur Übersicht</Button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Kosten bearbeiten</h1>
          <p className="text-sm text-gray-500">Ändern Sie die Details der Kosten #{kostenItem.id}.</p>
        </div>
      </div>
