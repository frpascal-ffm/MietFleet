...
export function FahrerBearbeiten() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { getFahrer, updateFahrer, fahrzeuge } = useData()
  ...
  useEffect(() => {
    if (fahrer) {
      setName(fahrer.name)
      setPhone(fahrer.phone || "")
      setEmail(fahrer.email || "")
      setStatus(fahrer.status)
      setDefaultCarId(fahrer.defaultCarId || "")
      setNotes(fahrer.notes || "")
    }
  }, [fahrer])
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Fahrer bearbeiten</h1>
          <p className="text-sm text-gray-500">Ändern Sie die Details des Fahrers {fahrer.name}.</p>
        </div>
      </div>
